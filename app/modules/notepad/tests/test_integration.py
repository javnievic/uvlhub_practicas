...
def test_get_notepad(test_client):
    ...
    response = test_client.post(
        "/notepad/create",
        data={"title": "Notepad2", "body": "This is the body of notepad2."},
    ...
    with test_client.application.app_context():
        ...
        notepad = Notepad.query.filter_by(
            title="Notepad2", user_id=current_user.id
        ).first()
...
def test_edit_notepad(test_client):
    ...
    response = test_client.post(
        "/notepad/create",
        data={"title": "Notepad3", "body": "This is the body of notepad3."},
        follow_redirects=True,
    )
    assert response.status_code == 200
    ...
    with test_client.application.app_context():
        ...
        notepad = Notepad.query.filter_by(
            title="Notepad3", user_id=current_user.id
        ).first()
        assert notepad is not None, "Notepad was not found in the database."
    ...
    with test_client.application.app_context():
        ...
        assert (
            notepad.body == "This is the edited body of notepad3."
        ), "The notepad body was not updated."
...
def test_delete_notepad(test_client):
    ...
    response = test_client.post(
        "/notepad/create",
        data={"title": "Notepad4", "body": "This is the body of notepad4."},
        follow_redirects=True,
    )
    assert response.status_code == 200
    ...
    with test_client.application.app_context():
        ...
        notepad = Notepad.query.filter_by(
            title="Notepad4", user_id=current_user.id
        ).first()
        assert notepad is not None, "Notepad was not found in the database."
